...
CREATE TABLE `Comment` (
    `comment_id` INT NOT NULL AUTO_INCREMENT, `post_id` INT NOT NULL, `user_id` int NOT NULL, `content` VARCHAR(45) NULL, `date` DATETIME NULL, PRIMARY KEY (`comment_id`), KEY `Comment_post_id_DongStory_post_id_fk_idx` (`post_id`), CONSTRAINT `Comment_post_id_DongStory_post_id_fk` FOREIGN KEY (`post_id`) REFERENCES `DongStory` (`post_id`), KEY `Comment_user_id_userinfo_user_id_fk_idx` (`user_id`), CONSTRAINT `Comment_user_id_userinfo_user_id_fk` FOREIGN KEY (`user_id`) REFERENCES `userinfo` (`user_id`)
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci;